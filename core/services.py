...
from project.settings import SCOPES, REDIRECT_URI, GOOGLE_OAUTH2_CLIENT_SECRETS_JSON, GMAIL_CLIENT_ID, \
...
class GoogleService:
    ...
    def receive_emails(cls, request):
        ...
        response = gmail_service.users().messages().list(userId='me', labelIds=[tags[0].name]).execute()
        ...
        if not messages:
            ...
            for message in messages:
                ...
                Message.objects.create(service=service[0], tag=tags[0], text=msg['snippet'],
...
class OAuthAuthorization:
    ...
    def gmail_authorization(cls, code):
        ...
        if code and not token:
            ...
            Token.objects.create(service=service[0], access_token=credentials.access_token,
